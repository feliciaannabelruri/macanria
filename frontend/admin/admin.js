...
(function () {
  ...
  function uploadImage(file) {
    return fetch("/api/upload", { method: "POST", credentials: "same-origin", headers: { "Content-Type": file.type || "application/octet-stream", "x-filename": file.name || "upload" }, body: file })
      .then(guard)
      .then(function (r) { return r.json().then(function (j) { return { ok: r.ok, j: j }; }); })
      .then(function (res) {
...
(function () {
  ...
  C.renderParallax = function (px, host) {
    ...
    var sc = card();
    sc.appendChild(head("Kehalusan transisi (global)"));
    var sv = (px.smooth != null && px.smooth !== "") ? px.smooth : "0.15";
    sc.appendChild(field("Kehalusan (0.05 = sangat mulus/berat, 1 = instan/tanpa smoothing)", sv, { key: "smooth", full: true, max: 6 }));
    host.appendChild(sc);
    host._collect = function () {
      return {
...
/* ===== Hamburger mobile nav (ditambahkan) ===== */
(function () {
  "use strict";
  function __hamburgerInit() {
    var sidebar = document.querySelector(".sidebar");
    var topbar = document.querySelector(".topbar");
    if (!sidebar || !topbar || document.querySelector(".hamburger")) return;
    ...
    // tutup saat memilih section
    sidebar.addEventListener("click", function (e) {
      if (e.target && e.target.classList && e.target.classList.contains("navlink")) close();
    });
